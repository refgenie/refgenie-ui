...
function Genome() {
  ...
  const archivesData = archives.map(query => query.data).filter(Boolean).flat();
  ...
  const combinedAssets = archivesData?.map((archive: any)  => {
    ...
    const matchingAssetGroup = assetGroups?.find((assetGroup: any) => assetGroup.id === matchingAsset?.asset_group_id);
      ...
      <div className='row p-2 p-lg-4 mt-4 mt-lg-0'>
        <div className='col-12'>
          ...
          <div className='d-flex align-items-center'>
            ...
            >
              {copied ? (
                ...
                <><i className='bi bi-clipboard-fill me-1' />Genome Digest</>
              )}
            </button>
            <a 
          ...
          <>
            ...
            <p className='text-ss mb-0'><strong>Genome Name: </strong>{alias[0]?.name}</p>
            <p className='text-ss mb-0'><strong>Genome Digest: </strong>{digest}</p>
            <p className='text-ss mb-0'><strong>Genome Description: </strong>{genome.description}</p>
          </>
          }
              ...
              <div className='row row-cols-1'>
                {combinedAssets.flatMap((group: any, index: number) =>                 
                  <div className='col mb-3' key={index}>
                    <div className='card asset-card cursor-pointer bg-body-tertiary shadow-sm' onClick={() => navigate(`/genomes/${digest}/${group.asset.digest}`)}>
                      <div className='card-body'>
                        <h6 className='fw-bold'>{group.assetGroup.name} / {group.asset.name}</h6>
                        <div className='text-xs'>

                          <p className='mb-2 fst-italic text-muted'>{group.asset.description}</p>
                          <span><strong>File Contents: </strong></span>
                          {group.archive.directory_contents.map((item: any, itemIndex: number) => (
                            <div className='text-muted' key={itemIndex}>{item}</div>
                          ))}

                          <div className='mt-1 d-flex align-items-end'>
                            <span><strong>Asset Digest: </strong><span className='text-muted'>{group.asset.digest}</span></span>
                            <span className='ms-auto d-flex align-items-center'><strong className='me-1'>Download ({(group.archive.size / 1024 / 1024 / 1024).toFixed(2)} gb): </strong><a 
