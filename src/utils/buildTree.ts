import type { LevelColType, LevelMapType, TreeNode, RawDataRow } from '../../types'
...
function buildSubtree(data: RawDataRow, levelCol: LevelColType): TreeNode[] | null {
  ...
  const children = uniqueTaxa.map(taxon => {
    ...
    const subsetData = data.filter((row: RawDataRow) => row[levelCol] == taxon && row[levelCol] != null)
